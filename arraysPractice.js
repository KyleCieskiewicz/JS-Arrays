...
function divider(numbersArray) {
  ...
  for (var i = 0; i < numbersArray.length; i++) {
    if (numbersArray[i] % 2 === 0) {
      arr[0].unshift(numbersArray[i]);
    }
    else if (!numbersArray[i] % 2 ===0) {
      arr[1].unshift(numbersArray[i]);
    }
  }
  return arr;
}
...
//array after adding ten to each item in numbers. *Verify your answer is correct. --> [15, 19, 26, 29, 35, 44, 58]
  ...
  //Code Here
  function addTen (arr) {
    var addedTen = numbers.map(function(ten) {
    return +ten + 10;
    });
    return addedTen;
  }
...
for(var i = 0; i < devMountainEmployees.length; i++) {
  if (devMountainEmployees[1] === devMountainEmployees[i]) {
...
var user3 = {
  ...
  password: 'russellLovesJavaScript',
  username: 'shortLoop'
};
